...
exports.protect = catchAsync(async (req, res, next) => {
  ...
  if (!currentUser) {
    return next(
      new AppError(
        'The user that belonging to this token does no longer exist.',
        401
      )
    );
  }
...
//da li ovo moze da radi u reactu
...
exports.forgotPassword = catchAsync(async (req, res, next) => {
  ...
  if (!user) {
    return next(
      new AppError('There is no user with that email. Please try again.', 404)
    );
  }
